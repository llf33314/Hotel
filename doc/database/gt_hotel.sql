...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_hotel` (
  ...
  `mark_modified` INT(1) NOT NULL DEFAULT 0 COMMENT '标记备注 0 默认开启 1 关闭 2 删除标记',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_room_category` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(45) NOT NULL COMMENT '房型名称',
  ...
  `discount_enable` TINYINT(2) NOT NULL DEFAULT 1 COMMENT '是否需要计算优惠价：0 需要 1 不需要',
  `discount` INT NOT NULL DEFAULT 100 COMMENT '折扣率范围 1 - 100 默认 100(计算方式：85/100=0.85 即85折)，即不打折',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_room` (
  ...
  `status` VARCHAR(15) NOT NULL COMMENT '房间状态 字典值 TODO: 后续给出编号  -- 可定义为字典值：在住、空房、脏房、已预订、已锁定、维护',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_file_record` (
  ...
  `name` VARCHAR(45) NOT NULL COMMENT '显示文件名称',
  `original_name` VARCHAR(45) NOT NULL COMMENT '文件上传时的原始名称',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`sys_dictionary_type` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `dict_type_name` VARCHAR(45) NOT NULL COMMENT '字典名称(例如：酒店设施)',
  `dict_type_status` INT NOT NULL DEFAULT 0 COMMENT '字典状态：0 使用中(默认) 1 不再使用',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`sys_dictionary` (
  ...
  `dict_code` INT UNSIGNED NOT NULL COMMENT '字典编码',
  `dict_name` VARCHAR(45) NOT NULL COMMENT '字典名称(例如：男女，比如状态，可以放在字典里，作为查看依据)',
  PRIMARY KEY (`id`),
  UNIQUE INDEX `dict_code_UNIQUE` (`dict_code` ASC)  COMMENT 'code 唯一标识')
ENGINE = InnoDB
COMMENT = '系统的字典数据表';
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_infrastructure_relation` (
  ...
  `updated_by` INT NOT NULL COMMENT '最后修改人 ID',
  `updated_at` TIMESTAMP NOT NULL COMMENT '最后修改时间',
  `display_value` VARCHAR(45) NULL COMMENT '展示值',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_hotel_setting` (
  ...
  `bulletin` VARCHAR(300) NULL COMMENT '公告',
  ...
  `Invoice_category` CHAR(20) NULL COMMENT '发票支持的类目 1 办公用品 2 住宿费 3 餐费 4 培训费 5 打球费 6 健身费 存储方式 1,2,3,4,5 or 1,2,3',
  `created_by` INT NOT NULL COMMENT '创建者ID',
  `created_at` DATETIME NOT NULL COMMENT '创建时间',
  `updated_by` INT NOT NULL COMMENT '最后修改人 ID',
  `updated_at` TIMESTAMP NOT NULL COMMENT '最后修改时间',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_infrastructure` (
  ...
  `module` CHAR(20) NOT NULL COMMENT '模块 hotel 酒店基础设施 room 客房基础设施',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_activity` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `hotel_id` INT NOT NULL COMMENT '酒店ID',
  `external_number` CHAR(16) NOT NULL COMMENT '外部活动编号 提供对外显示 ID 格式 前缀 HD+时间戳年月日时分秒毫秒 14位',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_activity_room` (
  ...
  `hotel_id` INT NOT NULL COMMENT '酒店ID',
  `activity_id` INT NOT NULL COMMENT '活动ID',
  `room_category_id` INT NOT NULL COMMENT '房型ID',
  `room_category_name` VARCHAR(45) NULL COMMENT '房型名称 冗余参数',
  `room_id` INT NOT NULL COMMENT '客房ID',
  `room_number` CHAR(15) NOT NULL COMMENT '客房编号',
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_reservation_consumer` (
  ...
  `id_type` VARCHAR(45) NOT NULL COMMENT '证件类型',
  `id_number` VARCHAR(45) NOT NULL COMMENT '证件编号',
  PRIMARY KEY (`id`))
ENGINE = InnoDB
COMMENT = '预订客户';
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_hotel_member_setting` (
  `hotel_id` INT NOT NULL COMMENT '酒店ID',
  `vip_level` TINYINT(2) NOT NULL DEFAULT 1 COMMENT '会员免押金',
  `vip_check_out` DATETIME NULL COMMENT '会员 最晚退房时间(默认退房时间均为离店当天14点)',
  `erp_logo` VARCHAR(300) NULL,
  PRIMARY KEY (`hotel_id`))
ENGINE = InnoDB
COMMENT = '酒店会员设置';
...
CREATE TABLE IF NOT EXISTS `gt_hotel`.`t_room_permanent` (
  ...
  `discount` INT NOT NULL DEFAULT 1 COMMENT '折扣 默认：1 无折扣',
