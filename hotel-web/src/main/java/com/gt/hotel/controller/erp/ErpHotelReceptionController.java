...
public class ErpHotelReceptionController extends BaseController {
     ...
     *
     * @param hotelId 酒店ID
     * @return ResponseDTO<ErpRoomStatusVo>
     */
    @ApiOperation(value = "状态统计", notes = "客房状态统计数")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "hotelId", value = "酒店ID", required = true, paramType = "path")
    })
    @GetMapping(value = "/{hotelId}/rooms/roomStatusStatistics", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseDTO<ErpRoomStatusVo> roomStatusStatistics(@PathVariable("hotelId") Integer hotelId) {
        return ResponseDTO.createBySuccess(this.roomService.findRoomsGroupStatistics(hotelId));
    }
     ...
     * @return ResponseDTO
     */
    @ApiOperation(value = "获取楼层列表", notes = "")
    @GetMapping(value = "/{hotelId}/floors", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseDTO getFloorList(@PathVariable("hotelId") Integer hotelId) {
        return ResponseDTO.createBySuccess(roomService.getFloorList(hotelId));
    }
    ...
    @PostMapping(value = "/{hotelId}/room/{roomId}", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
